...
omxc = Popen(['omxplayer', '-b','-o', 'local', movies[played_video]])
noiseVideo = Popen(['omxplayer', '--alpha', '255', noise])
...
#        print('preADC ', pre_adc_value,' - adc ', adcnow)
        if ((abs(adc_value - adc.read_adc(0, gain=GAIN))>50) and (not noiseVideo.poll()==None)):
            noiseVideo = Popen(['omxplayer','-o', 'local', '--vol', ' -4500', '--alpha', '55', noise], stdin=PIPE, stdout=PIPE, stderr=PIPE, close_fds=True)
    ...
    if ((abs(adc_value - pre_adc_value)>1000) or (not play_video == played_video)):
        ...
        if adc_value > 16000:
            played_video = 0
            omxc = Popen(['omxplayer', '-b', '-o', 'local', movies[0]],stdin=PIPE, stdout=PIPE, stderr=PIPE, close_fds=True)
        elif adc_value < 16000:
            played_video = 1
            omxc = Popen(['omxplayer', '-b', '-o', 'local', movies[1]],stdin=PIPE, stdout=PIPE, stderr=PIPE, close_fds=True)
        pre_adc_value = adc_value
